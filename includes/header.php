...
function getLaundryScheduleAla400() {
    ...
    // Mapeamento dos horários conforme a tabela fornecida
    $schedule = [
        1 => ['06:00 - 12:00'],    // Segunda: 6h às 12h
        2 => ['17:00 - 22:00'],     // Terça: 17h às 22h
        3 => ['12:00 - 17:00'],     // Quarta: 12h às 17h
        4 => ['06:00 - 12:00'],     // Quinta: 6h às 12h
        5 => ['17:00 - 22:00'],     // Sexta: 17h às 22h
        6 => ['12:00 - 17:00'],     // Sábado: 12h às 17h
        7 => ['12:00 - 17:00']      // Domingo: 12h às 17h
    ];
    ...
    return [
        ...
        'next_days' => [
            [
                'day_num' => ($dayOfWeek % 7) + 1,
                'day_name' => $daysMap[($dayOfWeek % 7) + 1],
                'schedule' => $schedule[($dayOfWeek % 7) + 1] ?? []
            ],
            [
                'day_num' => ($dayOfWeek % 7) + 2,
                'day_name' => $daysMap[($dayOfWeek % 7) + 2],
                'schedule' => $schedule[($dayOfWeek % 7) + 2] ?? []
            ]
        ]
    ];
}
...
<body>
  ...
  <div class="main-container">
    ...
    <div class="dashboard-grid">
      ...
      <!-- Novo Card para Lavanderia - Ala 400 -->
      <div class="card">
        <div class="card-header">
          <div class="card-icon">
        ...
        <div class="card-content">
          <div class="count">Ala 400</div>
          <p class="count-detail">Horários disponíveis</p>
          
          <div class="progress-container">
            <div class="progress-label">
              <span class="laundry-day"><?= $laundryInfo['today']['day_name'] ?></span>
            </div>
            <ul class="laundry-schedule">
              <?php if (!empty($laundryInfo['today']['schedule'])): ?>
                <?php foreach ($laundryInfo['today']['schedule'] as $timeSlot): ?>
                  <li><?= $timeSlot ?></li>
                <?php endforeach; ?>
              <?php else: ?>
                <li>Sem horário hoje</li>
              <?php endif; ?>
            </ul>
          </div>
            ...
            <ul class="laundry-schedule">
              <?php foreach ($laundryInfo['next_days'] as $day): ?>
                <?php if (!empty($day['schedule'])): ?>
                  <li>
                    <span class="laundry-day"><?= $day['day_name'] ?>:</span>
                    <?= implode(', ', $day['schedule']) ?>
                  </li>
                <?php endif; ?>
              <?php endforeach; ?>
            </ul>
